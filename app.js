...
function switchView(view) { document.querySelectorAll('.view').forEach(el => el.classList.toggle('active', el.id === view)); document.querySelectorAll('.nav-item').forEach(el => el.classList.toggle('active', el.dataset.view === view)); const names = {dashboard:'Buongiorno',orders:'Ordini tra sedi',inventory:'Magazzino',production:'Produzione',products:'Prodotti',expiry:'Scadenze',stocktake:'Inventario',hours:'Ore lavorate',temperatures:'Temperature',cleaning:'Pulizie',receipts:'Corrispettivi'}; document.querySelector('#pageTitle').textContent = names[view]; document.querySelector('#backHomeButton').classList.toggle('hidden', view === 'dashboard'); document.querySelector('main').scrollTo({ top: 0, behavior: 'smooth' }); }
...
document.querySelectorAll('.nav-item,[data-view-link]').forEach(button => button.addEventListener('click', () => { const view = button.dataset.view || button.dataset.viewLink; const protectedViews = ['temperatures', 'cleaning', 'receipts', 'hours']; if (selectedLocation === 'marotta' && protectedViews.includes(view) && sessionStorage.getItem('marotta-controls-authorized') !== 'true') { pendingProtectedView = view; document.querySelector('#marottaAccessForm').reset(); document.querySelector('#marottaAccessError').classList.add('hidden'); document.querySelector('#marottaAccessDialog').showModal(); return; } switchView(view); button.closest('details')?.removeAttribute('open'); }));
